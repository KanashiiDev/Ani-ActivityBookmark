...
function set(t,e,n){if(!t)throw new SyntaxError("'tag' not defined");// In case you forget
...
catch(e){alert("LocalStorage, required for saving settings, is not available.");return}

//get username
let auth;
let user = '';
let userid = 0;
try{auth=JSON.parse(localStorage.getItem("auth"))}
catch(e){console.warn("could not get auth");return}
if(auth)user=auth.name,userid=auth.id;
else try{user=document.querySelector(".nav .links .link[href^='/user/']").href.match(/\/user\/(.*)\//)[1]}
catch(e){alert("Please login before to use -Activity Saver- script.")}
...
interval = null;
var button=create("li",{class:"el-dropdown-menu__item mainbtn",id:"Saved Activities"},"Saved Activities");button.onclick=()=>{getSavedDiv()};
...
function removeactivity(id) {
  if(mainarray.length == 1) {mainarray = [];}
...
function creatediv() {
  ...
  var listDiv2=create("div",{class:"maindiv",id:"listDiv2"},"<b>"+button.innerText+"</b>");
  let expandbtn=create("button",{class:"mainbtns",id:"expandbtn",style:{background:"transparent"}});
  expandbtn.insertBefore(svg.expand.cloneNode(true), expandbtn.children[0]);
  expandbtn.onclick=()=>{expandDiv()};
  var list = document.querySelector(".activity-feed-wrap + div");
  listDiv2.appendChild(expandbtn);
  list.insertBefore(listDiv2, list.children[0]);
  var activitydataDiv=create("div",{class:"activitydataDiv",id:"activitydataDiv"});
...
activitydataDiv.setAttribute("class", "activitydataDiv expanded2");
  ...
  else {let x = document.querySelector(".activity-feed-wrap + div");x.insertBefore(listDiv2, x.children[0]);
        expandbtn.innerHTML = "";
         expandbtn.insertBefore(svg.expand.cloneNode(true), expandbtn.children[0]);
...
function getSavedDiv() {
    ...
    if (activefilter) {delay(1000).then(() => activefilter.className = activefilter.className.replace(/(?:^|\s)active(?!\S)/g, ''));}
    if (active) {creatediv();buildactivity();}
    if (!active) {closeDiv();}
}
...
function addSavetoActivities() {
    ...
    setTimeout(addSavetoActivities, 1000);
...
fetch(url,options).then(handleResponse).then(handleData).catch(handleError);let APIlimit=90,APIcallsUsed=0,APIcallsUsed_shortTerm=0,pending={},APIcounter=setInterval((function(){APIcallsUsed=0}),6e4),APIcounter2=setInterval((function(){APIcallsUsed_shortTerm=0}),1e4);function handleResponse(e){APIlimit=e.headers.get("x-ratelimit-limit"),APIcallsUsed=APIlimit-e.headers.get("x-ratelimit-remaining");try{return e.json().then((function(t){return e.ok?t:Promise.reject(t)}))}catch(t){throw console.warn(t,e),t}}
    function handleData(data) {
      ...
      let aimg=create("a",{class:"activitydataimg",id:"activitydataimg",href:"https://anilist.co/user/"+activity.user.name,style:{backgroundImage:"url("+activity.user.avatar.medium+")"}});
        ...
        let actlink=create("a",{class:"activitylink",id:activity.id,href:activity.siteUrl});
        let actremove=create("a",{class:"activitylink",id:activity.id});actremove.onclick=()=>{removeactivity(id)};
        ...
        let timeWrapper = create2("div","acttime",false,actusername);
				let time = nativeTimeElement(activity.createdAt);timeWrapper.appendChild(time);
        ...
        actionLikes.onclick = function(){location.href=activity.siteUrl}
        ...
        actionReplies.onclick = function(){if(replyWrap.style.display === "none"){replyWrap.style.display = "block"}else{replyWrap.style.display = "none"}
          ...
          replyLikeButton.onclick = function(){location.href=activity.siteUrl}
					replyLikeButton.appendChild(svg.likeNative.cloneNode(true));
					replyLikeButton.title = rep.likes.map(a => a.name).join("\n");
          if(rep.likes.some(like => like.name === username)){replyLikeButton.classList.add("liked")}
          ...
          let replyMarkdown = create2("div","reply-markdown",false,reply);
					...
					markdown.innerHTML = DOMPurify.sanitize(rep.text);
          markdown.innerHTML = makeHtml(rep.text);
      })
    }
}}
function handleError(error) {console.error(error);}
