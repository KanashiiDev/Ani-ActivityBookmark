...
function creatediv() {
	...
	if(!accessToken) {
		let loginLink = create("a", {class: "mainbtns",id: "signIn",href: "https://anilist.co/api/v2/oauth/authorize?client_id=12455&response_type=token"}, "<b>Sign In</b>");
...
function settingsDiv() {
    ...
    if(accessToken.length < 5) {localStorage.setItem("actautosave", autosave);
    SavetoBtn.classList.toggle("btn-active", JSON.parse(localStorage.getItem("actautosave")));}
...
function htmlfix(text) {
	...
	actfixtext = makeHtml(fix).replace(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/gm, '<blockquote>' + "$2" + '</blockquote>').replace(/(?<![a-z&])#/g, "").replace(/(<img.*)(<a)/g, "$1<br>$2");
